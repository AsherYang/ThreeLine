...
receiveFlg 调整为根据order_status处理
"""
class GetMyOrderListHandler(tornado.web.RequestHandler):
    def get(self, *args, **kwargs):
        ...
        if sign == md5Util.md5Signature(time):
            getOrder = GetOrder()
            netOrderList = getOrder.getMyOrderList(userId, orderStatus, page, size)
            if netOrderList:
                baseResponse.code = ResponseCode.op_success
                baseResponse.desc = ResponseCode.op_success_desc
                orderCount = getOrder.getMyOrderSizeByStatus(user_id=userId, order_status=orderStatus)
                page_total = (orderCount / size) + (1 if orderCount % size > 0 else 0)
